      ...
      <div class="chart-card">
        ...
        <div class="chart" ref="chartRef">
          <svg v-if="chartData.points.length" :width="chartWidth" :height="chartHeight">
            ...
            <polyline v-for="(line, idx) in chartData.extraLines" :key="idx" :points="line" fill="none" stroke="#7bd6ff" stroke-width="2" />
          </svg>
...
const stats = reactive({ pending: 1,pproved: 1}) // 可自定义默认值

// 简易 SVG 折线图（占位，可替换任意图表库）
...
const loadChart = async () => {
  ...
  if (res && res.data && Array.isArray(res.data.series)) {
    ...
    if (series.length) {
      ...
      const maxY = Math.max(...series.map(v => v)) || 1
...
const onExport = async () => {
  ...
  if (res && res.url) { window.open(res.url); return }
  if (res && res.blob) {
    const url = URL.createObjectURL(res.blob); const a = document.createElement('a'); a.href = url; a.download = '移模审批.xlsx'; a.click(); URL.revokeObjectURL(url)
  }
}
const onImport = async (e) => { const f = e.target.files && e.target.files[0]; if (f) await importMoveApprove(f) }
...
const onQuery = () => { /* 预留查询逻辑 */ }
const onReset = () => { Object.keys(form).forEach(k => form[k] = '') }

onMounted(async () => { await loadStats(); await loadChart() })

// 删除确认弹窗
const confirm = reactive({ visible: false })
const openDeleteConfirm = () => { confirm.visible = true }
const closeConfirm = () => { confirm.visible = false }
const confirmDelete = () => {
  // TODO: 这里对接后端批量删除接口
  closeConfirm()
}
